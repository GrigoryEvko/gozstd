...
func TestErrorTypeChecking(t *testing.T) {
	...
	// Test positive cases
	if !IsBufferError(bufErr) { t.Error("IsBufferError failed") }
	if !IsMemoryError(memErr) { t.Error("IsMemoryError failed") }
	if !IsParameterError(paramErr) { t.Error("IsParameterError failed") }
	if !IsDictionaryError(dictErr) { t.Error("IsDictionaryError failed") }
	if !IsCorruptionError(corrErr) { t.Error("IsCorruptionError failed") }
	if !IsStreamStateError(streamErr) { t.Error("IsStreamStateError failed") }
	if !IsVersionError(verErr) { t.Error("IsVersionError failed") }
	if !IsFrameError(frameErr) { t.Error("IsFrameError failed") }
	
	// Test negative cases (cross-checks)
	if IsMemoryError(bufErr) { t.Error("False positive: IsMemoryError on BufferError") }
	if IsBufferError(memErr) { t.Error("False positive: IsBufferError on MemoryError") }
	if IsParameterError(dictErr) { t.Error("False positive: IsParameterError on DictionaryError") }
}
